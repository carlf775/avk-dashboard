<script lang="ts">
  ...
  type SegDetection = { class: string; class_id: number; confidence: number; bbox: number[]; polygon: number[][] };
  type SegResult    = { image: string; width: number; height: number; detections: SegDetection[]; n_defects: number };
  type SegSummary   = { model: string; images: number; total_detections: number; images_with_defects: number; images_clean: number; defect_rate_pct: number; results: SegResult[] };

  let segData      = $state<SegSummary | null>(null);
  let activeFilter = $state<string | null>(null);

  const segCards = $derived(() => {
    const all = segData?.results ?? [];
    if (!activeFilter) return all;
    if (activeFilter === '__clean__') return all.filter(c => c.n_defects === 0);
    return all.filter(c => c.detections.some(d => d.class === activeFilter));
  });

  let lightboxIdx  = $state<number | null>(null);
  const lightboxCard = $derived(lightboxIdx !== null ? segCards()[lightboxIdx] : null);
  function openLightbox(idx: number) { lightboxIdx = idx; }
  function navigateGallery(dir: 1 | -1) {
    if (lightboxIdx === null) return;
    lightboxIdx = (lightboxIdx + dir + segCards().length) % segCards().length;
  }
  ...
  const CLASS_COLORS: Record<string, string> = {
    'black speck': '#ff5050', 'dent': '#ffa500', 'dimples': '#8a2be2',
    'discoloration': '#ffff00', 'groove': '#00c8ff', 'sharpie': '#ffb6c1',
    'smiley': '#00ff80', 'stripes': '#f97316',
  };

  onMount(async () => {
    const r = await fetch(import.meta.env.BASE_URL + 'rob_seg_results.json');
    if (r.ok) segData = await r.json();
  ...
  // Rolling line split into stable and degradation segments
  ...
  const hourlyMax = Math.max(...H_ANOM);
  function hourlyTicks() {
  ...
  <div class="lb" onclick={() => (lightboxIdx = null)}>
    ...
    <div class="lb-card" onclick={(e) => e.stopPropagation()}>
      <div class="lb-imgs-pair">
        <img src="{import.meta.env.BASE_URL}rob-seg-originals/{lightboxCard.image}" alt="Original" />
        <img src="{import.meta.env.BASE_URL}rob-seg-overlays/{lightboxCard.image}" alt="Heatmap" />
        <img src="{import.meta.env.BASE_URL}rob-seg-annotated/{lightboxCard.image}" alt="Segmentation" />
      </div>
      <div class="lb-meta">
        {lightboxCard.image.slice(0,12)}…
        {#if lightboxCard.n_defects > 0}
          {#each lightboxCard.detections as det}
            <span class="lb-det-tag" style="background:{CLASS_COLORS[det.class] ?? '#888'}22;color:{CLASS_COLORS[det.class] ?? '#888'};border-color:{CLASS_COLORS[det.class] ?? '#888'}55">
              {det.class} {(det.confidence * 100).toFixed(0)}%
            </span>
          {/each}
        {:else}
          <span class="score-good">CLEAN</span>
        {/if}
        · {lightboxIdx! + 1} / {segCards().length} · ← → to navigate · Esc to close
      </div>
    </div>
...
<div class="container">
  ...
  <div class="cards">
    ...
    <div class="card">
      ...
      <div class="sub">{goodPct}% · brightness &lt; 179</div>
    </div>
  </div>
  ...
  <div class="section">
    ...
    <div class="chart-scroll">
      <svg viewBox="0 0 {DW} {DH}" width="{DW}" style="min-width:{DW}px;height:auto;display:block">
        ...
        <!-- Rolling anomaly rate line (right axis) — split at degradation -->
        {#if rollingStablePts}
          <polyline points={rollingStablePts} fill="none" stroke="#70c1a3" stroke-width="2.5" stroke-linejoin="round"/>
        {/if}
        {#if bridgePts}
          <polyline points={bridgePts} fill="none" stroke="#70c1a3" stroke-width="1.5" stroke-dasharray="4,3" stroke-linejoin="round"/>
        {/if}
        {#if rollingDegradPts}
  ...
  <div class="section">
    <h2>Instance Segmentation Gallery — {segCards().length} Images</h2>
    <p class="chart-sub" style="margin-bottom:16px">
      Model: {segData?.model ?? 'unknown'} · {segData?.total_detections ?? 0} detections · {segData?.images_with_defects ?? 0} with defects · {segData?.images_clean ?? 0} clean · Click to enlarge · ← → to navigate.
    </p>
    {#if segData}
      <!-- Class legend / filter -->
      <div class="seg-legend">
        {#each Object.entries(CLASS_COLORS) as [cls, col]}
          <button
            class="seg-class-pill"
            class:seg-pill-active={activeFilter === cls}
            style="background:{activeFilter === cls ? col + '44' : col + '22'};color:{col};border-color:{activeFilter === cls ? col : col + '55'}"
            onclick={() => toggleFilter(cls)}
          >{cls}</button>
        {/each}
        <button
          class="seg-class-pill seg-pill-clean"
          class:seg-pill-active={activeFilter === '__clean__'}
          onclick={() => toggleFilter('__clean__')}
        >clean</button>
        {#if activeFilter}
          <button class="seg-pill-clear" onclick={() => { activeFilter = null; lightboxIdx = null; }}>✕ clear</button>
        {/if}
      </div>
      <div class="gallery-grid">
        {#each segCards() as card, idx}
          <div class="gallery-card" onclick={() => openLightbox(idx)}>
            <div class="gallery-imgs">
              <img src="{import.meta.env.BASE_URL}rob-seg-originals/{card.image}" alt="Original" loading="lazy" />
              <img src="{import.meta.env.BASE_URL}rob-seg-overlays/{card.image}" alt="Heatmap" loading="lazy" />
              <img src="{import.meta.env.BASE_URL}rob-seg-annotated/{card.image}" alt="Segmentation" loading="lazy" />
            </div>
            <div class="gallery-meta">
              <span class="gallery-ts">{card.image.slice(0,8)}…</span>
              {#if card.n_defects > 0}
                <span class="gallery-score score-anom">{card.detections[0].class} · {(card.detections[0].confidence*100).toFixed(0)}%</span>
              {:else}
                <span class="gallery-score score-good">CLEAN</span>
              {/if}
            </div>
          </div>
        {/each}
    ...
    {:else}
      <p class="chart-sub">Loading segmentation results…</p>
    {/if}
  </div>

  <footer>
    <strong>Classification:</strong> Anomaly = heatmap brightness ≥ 179 (0.7 × 255) · Good = brightness &lt; 179 ·
    Total inspections = total heatmaps · Period: 2025-09-30 – 2025-11-12 · Source: SurrealDB vision system database
  </footer>
...
<style>
  /* ── Theme (dark only — light handled by parent) ───────────────────────── */
  .wrap {
  ...
  .gallery-imgs { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 1px; background: var(--border); }
  .gallery-imgs img { width: 100%; aspect-ratio: 3/4; object-fit: cover; display: block; background: var(--surface2); }
  .gallery-meta {
  ...
  /* ── Segmentation gallery extras ───────────────────────────────────────── */
  .seg-legend { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 16px; }
  .seg-class-pill {
  ...
  .seg-class-pill:hover { opacity: 0.85; }
  .seg-pill-active { box-shadow: 0 0 0 1px currentColor; }
  .seg-pill-clean {
    color: #a1a1aa; border-color: #52525b; background: #27272a22;
  }
  .seg-pill-clean.seg-pill-active {
    color: #d4d4d8; border-color: #a1a1aa; background: #3f3f4644;
  }
  .seg-pill-clear {
    font-size: 11px; font-weight: 600; padding: 3px 10px;
    border-radius: 99px; border: 1px solid #52525b;
    color: #a1a1aa; background: none; cursor: pointer;
    transition: color 0.15s, border-color 0.15s;
  }
  .seg-pill-clear:hover { color: #e4e4e7; border-color: #71717a; }
  .lb-seg-img { width: 100%; max-height: 75vh; object-fit: contain; border-radius: 8px; display: block; }
  .lb-det-tag {
  ...
  .lb-imgs-pair { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 8px; }
